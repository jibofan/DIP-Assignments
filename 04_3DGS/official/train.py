...
# This script lives in the `official/` subfolder. Fix up the import path so that:
#   1) the local official/ modules (gaussian_model, gaussian_renderer) are loaded
#      from here and take precedence (insert at the front of sys.path), and
#   2) shared project-root modules such as data_utils.py are still importable
#      from the parent directory (append the parent to sys.path).
...
class GaussianTrainer:
    ...
    def save_debug_images(self, epoch: int, rendered_images: torch.Tensor,
        ...
        rendered = np.nan_to_num(rendered, nan=0.0, posinf=1.0, neginf=0.0)
    ...
    def load_checkpoint(self, path: str) -> int:
        ...
        with torch.no_grad():
            ...
            for name, p in self.model.named_parameters():
                ...
                if bad.any():
                    n_bad_params += int(bad.sum().item())
                    if 'rotation' in name:
                        ident = torch.zeros_like(p)
                        ident[..., 0] = 1.0
                        p[bad] = ident[bad]
                    else:
                        p[bad] = 0.0
            if n_bad_params > 0:
                print(f"[WARN] checkpoint contained {n_bad_params} non-finite param values; sanitized.")
    ...
    def train_step(self, batch: dict, in_train = True) -> float:
        ...
        if not torch.isfinite(loss):
            print(f"[WARN] non-finite loss ({loss.item()}); skipping optimizer step.")
        ...
        for p in self.model.parameters():
            if p.grad is not None and not torch.isfinite(p.grad).all():
                ...
                break
        if bad_grad:
            print("[WARN] non-finite grad; skipping optimizer step.")
            self.optimizer.zero_grad(set_to_none=True)
...
'''
# Run from the project root (the parent of official/).
# data_utils.py is expected to live in the project root; the official rasterizer
# modules live in official/. All outputs (checkpoints, debug_images/, the final
# debug_rendering.mp4) are written under --checkpoint_dir, i.e. data/chair/official.

python official/train.py \
    --colmap_dir data/chair \
    --checkpoint_dir data/chair/official \
    --save_every 1 --num_epochs 20

# Resume from a saved checkpoint:
python official/train.py \
    --colmap_dir data/chair \
    --checkpoint_dir data/chair/official \
    --resume data/chair/official/checkpoint_000019.pt
'''